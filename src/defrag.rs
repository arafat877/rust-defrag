...
/// defrag.rs â€” Defragmentation engine
///
...
///   - Never touch NTFS metadata files (`$MFT`, `$LogFile`, â€¦)
///   - Skip pagefile.sys / hiberfil.sys / swapfile.sys
///   - Skip files whose open returns `ERROR_SHARING_VIOLATION`
///   - On any move failure: log, skip, continue â€” never abort
...
pub fn defragment(
    ...
    for (idx, file_info) in targets.iter().enumerate() {
        ...
        // Final safety check â€” should already be filtered, but be defensive
...
fn defrag_single_file(
    ...
    // Move each run sequentially into [target_lcn â€¦ target_lcn + total_clusters)
