...
pub fn open_volume(path: &str) -> DefragResult<VolumeHandle> {
    ...
    #[cfg(not(target_os = "windows"))]
    {
        ...
        log::warn!("open_volume: non-Windows stub — volume operations disabled");
        Ok(VolumeHandle(0))
    }
}
...
pub fn get_volume_bitmap(vol: &VolumeHandle) -> DefragResult<(i64, i64, Vec<u8>)> {
    ...
    #[cfg(not(target_os = "windows"))]
    {
        // Stub — return a synthetic 1 M cluster bitmap, half free
...
pub fn get_retrieval_pointers(file_handle: isize) -> DefragResult<Vec<ClusterRun>> {
    #[cfg(target_os = "windows")]
    {
        ...
        let buf_size: usize = 1 << 16; // 64 KiB — enough for ~2700 extents
    ...
    #[cfg(not(target_os = "windows"))]
    {
        // Stub — synthetic fragmented file
        Ok(vec![
